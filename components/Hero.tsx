        ...
        if (banners.length > 0) {
          ...
          if (imageUrl && !imageUrl.startsWith("http")) {
            imageUrl = imageUrl.startsWith("/")
              ? `https://shivaay-backend.onrender.com${imageUrl}`
              : `https://shivaay-backend.onrender.com/${imageUrl}`;
          }
          ...
          if (videoUrl && !videoUrl.startsWith("http")) {
            videoUrl = videoUrl.startsWith("/")
              ? `https://shivaay-backend.onrender.com${videoUrl}`
              : `https://shivaay-backend.onrender.com/${videoUrl}`;
          }
  ...
  return (
    ...
    >
      ...
      <div className="hero-bg-container absolute inset-0 z-0">
        ...
        {banner?.videoUrl || banner?.imageUrl?.includes("video") ? (
          <video
            key={banner?.videoUrl || banner?.imageUrl}
          ...
          <video
            ...
            playsInline
            preload="auto"
            src="https://res.cloudinary.com/dppcmxdhi/video/upload/v1771392399/WhatsApp_Video_2026-02-16_at_13.06.54_nnybeo.mp4"
            className="w-full h-full object-cover grayscale-[0.2] brightness-110"
          />
        )}
      </div>
      ...
      <div className="relative z-20 text-center px-6">
        ...
        <h2 className="hero-title text-luxury tracking-[0.5em] uppercase text-[10px] md:text-xs mb-4 font-sans font-bold">
          {banner?.subtitle || "Fine Art Photographer"}
        </h2>
        ...
        <div className="hero-btn flex flex-col md:flex-row gap-8 justify-center items-center">
          <Link
            to="/portfolio"
            className="group relative px-12 py-5 overflow-hidden bg-charcoal"
          >
            <span className="relative z-10 text-white uppercase tracking-[0.3em] text-[10px] font-bold">
              Explore Gallery
            </span>
          ...
          <Link
            to="/contact"
            className="text-charcoal hover:text-luxury transition-colors uppercase tracking-[0.4em] text-[10px] font-bold border-b-2 border-charcoal/10 pb-1"
          >
            Book Your Story
          </Link>
        </div>
      ...
      <div
        onClick={() => window.lenis?.scrollTo("#featured-galleries")}
